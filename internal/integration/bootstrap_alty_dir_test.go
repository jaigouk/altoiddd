...
// ===========================================================================
// Scenario: alto init creates alto-scaffold/ directory files
// ===========================================================================
...
func TestInitCreatesAltoConfigTOML_GivenEmptyProject_WhenPreviewConfirmExecute_ThenConfigTOMLExists(t *testing.T) {
	...
	// Then: alto-scaffold/config.toml exists with valid TOML
	configPath := filepath.Join(dir, "alto-scaffold", "config.toml")
	content, err := os.ReadFile(configPath)
	require.NoError(t, err, "alto-scaffold/config.toml should exist after Execute")

	var parsed map[string]any
	_, err = toml.Decode(string(content), &parsed)
	require.NoError(t, err, "alto-scaffold/config.toml should contain valid TOML")
...
func TestInitCreatesKnowledgeIndex_GivenEmptyProject_WhenPreviewConfirmExecute_ThenIndexTOMLExists(t *testing.T) {
	...
	// Then: alto-scaffold/knowledge/_index.toml exists with valid TOML
	indexPath := filepath.Join(dir, "alto-scaffold", "knowledge", "_index.toml")
	content, err := os.ReadFile(indexPath)
	require.NoError(t, err, "alto-scaffold/knowledge/_index.toml should exist after Execute")
...
func TestInitCreatesDocRegistry_GivenEmptyProject_WhenPreviewConfirmExecute_ThenRegistryTOMLExists(t *testing.T) {
	...
	// Then: alto-scaffold/maintenance/doc-registry.toml exists with valid TOML
	registryPath := filepath.Join(dir, "alto-scaffold", "maintenance", "doc-registry.toml")
	content, err := os.ReadFile(registryPath)
	require.NoError(t, err, "alto-scaffold/maintenance/doc-registry.toml should exist after Execute")
...
func TestInitSkipsExistingAltoFiles_GivenProjectWithAltoConfig_WhenPreviewConfirmExecute_ThenFileNotOverwritten(t *testing.T) {
	...
	// Given: project with existing alto-scaffold/config.toml
	app := newApp(t)
	dir := makeTempProjectDir(t)
	altoDir := filepath.Join(dir, "alto-scaffold")
	...
	for _, action := range preview.FileActions() {
		if action.Path() == "alto-scaffold/config.toml" {
			assert.Equal(t, vo.FileActionSkip, action.ActionType(),
				"alto-scaffold/config.toml should be marked as Skip when it already exists")
		}
	}
	...
	assert.Equal(t, originalContent, string(content),
		"existing alto-scaffold/config.toml should not be overwritten")
}
...
func TestInitCreatesAllThreeAltoFiles_GivenEmptyProject_WhenFullFlow_ThenAllExist(t *testing.T) {
	...
	// Then: all three alto-scaffold files exist
	expectedFiles := []string{
		filepath.Join(dir, "alto-scaffold", "config.toml"),
		filepath.Join(dir, "alto-scaffold", "knowledge", "_index.toml"),
		filepath.Join(dir, "alto-scaffold", "maintenance", "doc-registry.toml"),
	}
