...
func TestDriftDetectionAdapter_Detect(t *testing.T) {
	...
	t.Run("returns empty report when no meta files", func(t *testing.T) {
		t.Parallel()
		tmpDir := t.TempDir()
		kbDir := filepath.Join(tmpDir, "alto-scaffold", "knowledge", "tools", "test-tool")
	...
	t.Run("detects stale entry from last_verified", func(t *testing.T) {
		t.Parallel()
		tmpDir := t.TempDir()
		kbDir := filepath.Join(tmpDir, "alto-scaffold", "knowledge", "tools", "claude-code")
		require.NoError(t, os.MkdirAll(kbDir, 0o755))
...
`
	...
	t.Run("skips malformed toml gracefully", func(t *testing.T) {
		t.Parallel()
		tmpDir := t.TempDir()
		kbDir := filepath.Join(tmpDir, "alto-scaffold", "knowledge", "tools", "bad-tool")
		require.NoError(t, os.MkdirAll(kbDir, 0o755))
	...
	t.Run("scans multiple tools", func(t *testing.T) {
		...
		for _, tool := range []string{"tool-a", "tool-b"} {
			kbDir := filepath.Join(tmpDir, "alto-scaffold", "knowledge", "tools", tool)
			require.NoError(t, os.MkdirAll(kbDir, 0o755))
...
`
	...
	t.Run("entry path contains tool name", func(t *testing.T) {
		t.Parallel()
		tmpDir := t.TempDir()
		kbDir := filepath.Join(tmpDir, "alto-scaffold", "knowledge", "tools", "cursor")
		require.NoError(t, os.MkdirAll(kbDir, 0o755))
