...
5. If vibe-seed wanted to auto-generate architecture fitness function tests from a bounded context map, what libraries would it build on?

## 2. Context

vibe-seed generates DDD project scaffolding including bounded context maps, layer structure
...
**Recommended approach for vibe-seed:**
...
1. vibe-seed reads bounded context map (YAML)
...
1. vibe-seed generates `tests/architecture/test_layers.py`
...
Use **import-linter** for generated architecture fitness functions in vibe-seed scaffolding.

**Rationale:**
- Actively maintained (released Feb 6, 2026 — days before this research)
- Permissive license (BSD-2)
- Config-driven: vibe-seed can generate TOML contracts from a bounded context map without
  any runtime Python code generation
- Runs standalone CLI (`lint-imports`) and integrates with `uv run`
- Supports exactly the DDD layer pattern vibe-seed uses (domain/application/infrastructure)
...
**Rationale:**
- Rules are Python code → composable with vibe-seed's Python code generation
- Apache-2.0 license
...
No tool auto-generates fitness function tests from a bounded context map. vibe-seed would be
