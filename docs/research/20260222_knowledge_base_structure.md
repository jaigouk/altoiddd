...
#### Notable limitations for alto

- No subagent/agent concept (uses rules, not personas)
- Global config is SQLite, not file-based -- alto cannot generate global config
...
### Implications for `alto detect`

1. Claude Code and Roo Code have **file-based** global configs that alto can detect and compare.
2. Cursor has **SQLite-based** global config -- alto can detect the DB file exists but
...
**Implication for alto:** Generate both `AGENTS.md` and tool-specific configs. AGENTS.md
...
## 4. Proposed `.alto/knowledge/` Directory Structure
...
- On `alto init`, files are copied from `current/` into the project.
...
```
alto://knowledge/tools/{tool}/{subtopic}?version={version}
alto://knowledge/ddd/{topic}
alto://knowledge/conventions/{topic}
alto://knowledge/cross-tool/{topic}
```
...
```bash
alto kb tools/claude-code/agent-format              # Current version
alto kb tools/claude-code/agent-format --version v2.0  # Specific version
alto kb ddd/aggregates                               # DDD reference
alto kb cross-tool/concept-mapping                   # Cross-tool mappings
```

### MCP Resource Resolution

```python
@mcp.resource("alto://knowledge/tools/{tool}/{subtopic}")
...
| **Version mismatch** | `verified_against` vs installed tool version | Compare with `alto detect` output |
...
### `alto doc-health` Integration

```bash
alto doc-health --knowledge
...
```
alto detect (installed tools + versions)
    |
    v
...
DriftReport (list of stale/mismatched/deprecated entries)
    |
    v
alto doc-health --knowledge (human-readable report)
```
...
- **Drift detection ready**: Every entry carries verification metadata. `alto doc-health --knowledge`
...
|---|-------|------|----------------|----------|-------------|
| 1 | Implement `.alto/knowledge/` directory scaffolding | Task | Knowledge Base | P0 | Create the directory structure, `_index.toml`, and `_meta.toml` files for all 4 tools. Initial content for `current/` version of each tool. |
...
| 8 | Implement `alto kb` CLI command | Task | CLI Framework | P0 | Wire `alto kb <topic>` to KnowledgeLookupPort. Rich output for terminal. |
| 9 | Implement drift detection metadata validation | Task | Knowledge Base | P1 | `alto doc-health --knowledge` reads `[_meta]` sections, compares with `alto detect` output. |
...
| 11 | Implement MCP resource for knowledge lookup | Task | MCP Framework | P1 | Wire `alto://knowledge/*` MCP resources to KnowledgeLookupPort. |
