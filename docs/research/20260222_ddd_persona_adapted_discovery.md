...
How should vibe-seed adapt DDD discovery questions for different user personas
(technical vs non-technical), and what plain language equivalents exist for core
DDD terminology?

## Context

vibe-seed's PRD defines five personas (PRD Section 3):
...
**Key technique for vibe-seed:** The "who-does-what-with-what-why" framework
...
**Adaptation for text-based CLI:** Since vibe-seed is text-only (no visual
...
**Text-based equivalent for vibe-seed:**
...
**Relevance to vibe-seed:** Steps 1-5 map to the guided question flow. Steps
6-8 map to artifact generation and ticket creation. The process is explicitly
"not linear" -- vibe-seed should allow jumping between phases.
...
**Relevance to vibe-seed:** Phase 1 questions ("What problem? Who? What
...
**Gap Qlerify fills vs vibe-seed:** Qlerify uses visual EventStorming boards;
vibe-seed must achieve the same discovery through text-only conversation.
...
## Implementation Recommendations for vibe-seed

### 1. Persona Detection Strategy

Detect persona early in the flow (during `vs init` or `vs guide`):
...
Regardless of which register the questions use, vibe-seed should internally
