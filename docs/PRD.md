...
# Product Requirements Document: alto
...
**As a** Solo Developer, **I want to** describe my project idea and have alto guide me through domain discovery, **so that** I get a properly structured project without skipping DDD.
...
**As a** Team Lead, **I want to** apply alto conventions to an existing project, **so that** we get structured issue tracking and DDD alignment without starting over.

**Flow:**
1. Team lead runs `alto init --existing` in the project directory
2. alto **creates a new git branch** (`alto/init`) — all changes happen there, never on main
3. alto scans existing code, docs, configs, and folder structure
4. **Gap analysis** — identifies what's missing vs a fully-seeded project:
   ...
   - Existing files that conflict with alto defaults
5. Shows the gap report and proposed changes (preview, like `alto init`)
...
**Branch safety rules:**
- `alto init --existing` MUST be run in a clean git working tree (no uncommitted changes)
- All changes go to `alto/init` branch, never directly to current branch
- If branch already exists, abort with message (user must clean up or use `--force-branch`)
- User merges manually — alto never merges for you
- **Existing tests MUST pass** — after scaffolding, `alto init --existing` runs the project's existing test suite. If any test fails, it rolls back all changes on the branch and reports what broke. Zero test regressions is a hard gate.
...
2. alto generates tool-specific config files (`.claude/`, `.cursor/`, etc.)
...
**As a** Product Owner, **I want to** describe my product vision and have alto produce structured requirements and tickets, **so that** I can hand off to developers with clear scope and priorities.

**Flow:**
1. PO writes a 4-5 sentence product vision in README
2. alto proposes domain stories in business language (no DDD jargon)
...
2. alto identifies all open tickets that depend on or are siblings of the closed ticket
...
4. **Conflict resolution** — If alto wants to create a file that exists, rename ours: `filename_alto.md`
...
2. **Compare** — Check for conflicts between global settings and what alto wants to set locally
...
Global settings scan:
  ⚠ CONFLICT  ~/.claude/CLAUDE.md defines git rules that differ from alto defaults
              Global: "NEVER add Co-Authored-By lines"
              Local:  (alto would set the same — no conflict)
              → OK, compatible

  ⚠ CONFLICT  ~/.claude/settings.json has custom model preferences
              Global sets default model → sonnet
              Local:  alto has no model preference
              → OK, no conflict
...
| CLI name | `vs` (alto) | Short, memorable, unix-style |
...
| Conflict rename | If alto needs to create a conflicting file, suffix ours: `filename_alto.md` |
| Preview first | All file operations shown before execution (`alto init` dry-run) |
...
| Branch for existing | `alto init --existing` always creates a new branch, never writes to current branch |
| Clean tree required | `alto init --existing` refuses to run with uncommitted changes |
| Never merge | alto never merges branches — user merges manually after review |
| **Zero test regression** | `alto init --existing` runs existing test suite after scaffolding — if ANY test fails, roll back all changes and report. This is a **hard gate**, no exceptions. |
...
| Phase 7: Ticket & Doc Health | `alto ticket-health`, `alto doc-health`, drift detection, ripple review, maintenance registry |
