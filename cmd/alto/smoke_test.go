...
// binaryPath is the path to the compiled alto binary.
// Set by TestMain.
var binaryPath string

func TestMain(m *testing.M) {
	// Build binary to temp location for testing.
	tmpDir, err := os.MkdirTemp("", "alto-smoke-*")
	if err != nil {
		panic("creating temp dir: " + err.Error())
	}
	defer os.RemoveAll(tmpDir)

	binaryPath = filepath.Join(tmpDir, "alto")

	cmd := exec.Command("go", "build", "-o", binaryPath, ".")
	cmd.Dir = filepath.Join(".") // cmd/alto directory
...
// runAlto executes the alto binary with the given args and returns stdout, stderr, and exit code.
func runAlto(t *testing.T, args ...string) (stdout, stderr string, exitCode int) {
	t.Helper()
...
func TestSmoke_Version(t *testing.T) {
	stdout, stderr, exitCode := runAlto(t, "version")
...
func TestSmoke_Help(t *testing.T) {
	stdout, stderr, exitCode := runAlto(t, "help")
...
func TestSmoke_NoArgs(t *testing.T) {
	stdout, stderr, exitCode := runAlto(t)
...
func TestSmoke_Detect(t *testing.T) {
	stdout, stderr, exitCode := runAlto(t, "detect")
...
func TestSmoke_Check(t *testing.T) {
	stdout, stderr, exitCode := runAlto(t, "check")
...
func TestSmoke_PersonaList(t *testing.T) {
	stdout, stderr, exitCode := runAlto(t, "persona", "list")
...
func TestSmoke_KB(t *testing.T) {
	stdout, stderr, exitCode := runAlto(t, "kb")
...
func TestSmoke_DocHealth(t *testing.T) {
	stdout, stderr, exitCode := runAlto(t, "doc-health")
...
func TestSmoke_DocReviewList(t *testing.T) {
	stdout, stderr, exitCode := runAlto(t, "doc-review", "list")
...
func TestSmoke_InitExistingDryRun(t *testing.T) {
	...
	stdout, stderr, exitCode := runAlto(t, "init", "--existing", "--dry-run")
...
func TestSmoke_InvalidCommand(t *testing.T) {
	stdout, stderr, exitCode := runAlto(t, "nonexistent-command")
