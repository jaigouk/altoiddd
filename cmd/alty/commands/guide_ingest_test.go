...
func setupIngestTest(t *testing.T) (*composition.App, string, string) {
	...
	tmpDir := t.TempDir()
	altyDir := filepath.Join(tmpDir, ".alty")
	...
	return app, session.SessionID(), altyDir
}
...
func TestGuideAgentIngest_WhenPersonaAndAllAnswers_CompletesSession(t *testing.T) {
	t.Parallel()
	app, sessionID, altyDir := setupIngestTest(t)

	envelopes := allMVPAnswers(sessionID)
	ingestPath := writeJSONL(t, filepath.Dir(altyDir), envelopes)

	var out bytes.Buffer
	err := runGuideAgentIngest(context.Background(), app, ingestPath, altyDir, &out)
	require.NoError(t, err)
...
func TestGuideAgentIngest_WhenMismatchedSessionID_ReturnsError(t *testing.T) {
	t.Parallel()
	app, _, altyDir := setupIngestTest(t)
...
func TestGuideAgentIngest_WhenNoSessionFile_ReturnsError(t *testing.T) {
	...
	emptyDir := t.TempDir()
	altyDir := filepath.Join(emptyDir, ".alty")
	require.NoError(t, os.MkdirAll(altyDir, 0o755))
...
func TestGuideAgentIngest_WhenStdinDash_ReadsStdin(t *testing.T) {
	...
	var out bytes.Buffer
	err = runGuideAgentIngestFromReader(context.Background(), app, stdinReader, altyDir, &out)
	require.NoError(t, err)
...
func TestGuideAgentIngest_WhenInvalidJSON_ReturnsErrorWithLineNumber(t *testing.T) {
	...
	path := filepath.Join(filepath.Dir(altyDir), "bad.jsonl")
...
`, sessionID, sessionID)
	...
	var out bytes.Buffer
	err := runGuideAgentIngest(context.Background(), app, path, altyDir, &out)
	require.Error(t, err)
...
func TestGuideAgentIngest_WhenEmptyFile_EmitsCurrentStatus(t *testing.T) {
	...
	ingestPath := writeJSONL(t, filepath.Dir(altyDir), []map[string]interface{}{})
